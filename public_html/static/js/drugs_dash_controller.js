...
homeApp.controller('homeController',['$scope','$http',function($scope,$http){
    ...
    $scope.processAdverseEventsPerDrugResponse = function(data) {
        var barChartValues=[];
        var barChartLabels=[];
    ...
    $http.get('https://api.fda.gov/drug/event.json?search=receivedate:[20040101+TO+20150101]&count=patient.drug.medicinalproduct.exact&limit=10').success($scope.processAdverseEventsPerDrugResponse);
    };
    ...
    var barChartClickAction = function (d,element) { 
          ...
          $http.get('https://api.fda.gov/drug/event.json?search=receivedate:[20040101+TO+20150101]+AND+patient.drug.medicinalproduct:'+$scope.drugclicked+'&count=receivedate').success(processAdverseEventsSpikeForDrugResponse);
    };
    
    var processAdverseEventsSpikeForDrugResponse = function(data) {
        var lineChartValues=[];
        var lineChartLabels=[];
        lineChartValues.push('Reports'); // c3 needs this first in the array
        ...
        //Get the enforcement reports for this drug
        $http.get('https://api.fda.gov/drug/enforcement.json?search=product_description:'+$scope.drugclicked+'&count=report_date').success(processEnforcementReportsForDrugResponse).error(errorEnforcementReportsForDrugResponse);  

        //$scope.enforcementBarChartValues=['Recalls',100,200];
        //$scope.enforcementBarChartLabels=['When','201101','201301'];
        


        //generateLineChart('#theChart',lineChartValues,lineChartLabels);
        //generateLine2Chart('#theChart',lineChartValues,lineChartLabels,$scope.enforcementBarChartValues,$scope.enforcementBarChartLabels);
        generateDrugReportAndRecallChart('#theChart',lineChartValues,lineChartLabels,$scope.enforcementBarChartValues,$scope.enforcementBarChartLabels);
        $scope.setViewState('adversereports');
        $scope.doesdrughaverecalls=true;
    };
